<script>
    ...
    const API_URL = "https://sheetdb.io/api/v1/903blza7eaydi";
    ...
    // دالة التحقق (تم تحسينها لضمان الاستجابة)
    function verifyCode() {
        // تنظيف النصوص
        ...
        if (usedCodesLocal.includes(inputCode)) {
            msgEl.innerText = "❌ تم استخدام هذا الكود مسبقاً!";
            msgEl.style.color = "var(--danger)";
            btn.disabled = false;
            btn.innerText = "تفعيل";
            return;
        ...
        fetch(searchUrl)
            ...
            .then(data => {
                ...
                if (codeEntry.used === "true" || codeEntry.used === true || codeEntry.used === 1 || codeEntry.used === "yes") {
                    throw new Error("مستخدم");
                }

                // الكود صحيح -> تحديث السيرفر
                markCodeAsUsed(codeEntry.id, inputCode);
            })
            .catch(error => {
                ...
                if (error.message === "غير موجود") {
                    msgEl.innerText = "❌ كود التفعيل غير صحيح!";
                    msgEl.style.color = "var(--danger)";
                } else if (error.message === "كود خاطئ") {
                    msgEl.innerText = "❌ كود التفعيل غير صحيح!";
                    msgEl.style.color = "var(--danger)";
                } else if (error.message === "مستخدم") {
                    msgEl.innerText = "❌ هذا الكود مستخدم من قبل شخص آخر!";
                    msgEl.style.color = "var(--danger)";
                } else {
                    msgEl.innerText = "⚠️ خطأ في الاتصال بالسيرفر.";
                    msgEl.style.color = "var(--danger)";
                }
            });
    }
    ...
    // دالة تحديث الشيت
    function markCodeAsUsed(id, codeVal) {
        ...
        .then(response => {
            if(response.ok) {
                ...
                // فشل التحديث (لكن الكود صحيح، دع المستخدم يدخل لأجل التجربة أو أظهر خطأ)
                document.getElementById('activation-msg').innerText = "تم التفعيل (لم يتم تحديث السجل بسبب صلاحيات)";
                document.getElementById('activation-msg').style.color = "var(--success)";
                grantAccess(codeVal);
            }
        })
        .catch(() => {
            grantAccess(codeVal); // السماح بالدخول حتى لو فشل التحديث في حالة انقطاع الانترنت
        });
    }
    ...
    function grantAccess(usedCode) {
        ...
        // حفظ الكود محلياً
        let usedCodesLocal = JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY) || "[]");
        if(!usedCodesLocal.includes(usedCode)) {
            usedCodesLocal.push(usedCode);
